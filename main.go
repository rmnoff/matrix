...
func main() {
	...
	api.Post("/check/new/", func(c *routing.Context) error {
		...
		pastLifeBlocks = append(pastLifeBlocks, getAnswerFromTableIndistinct(db, mainLesson, 9, languageShort, gender))
		if languageShort == "ru" {
...
func getAnswerFromTableIndistinct(db *sqlx.DB, id string, tableNumber int, lang string, sex string, short_opt ...bool) Block {
	if id != "" {
		language := Language{}
		err := db.Get(&language, "SELECT * FROM predictionlang WHERE short = $1", lang)
		if err != nil {
		...
		query := fmt.Sprintf("SELECT * FROM prediction WHERE type_id=%d AND lang_id=%d AND id IN(SELECT prediction_id FROM predictionrel WHERE combination=%s)", tableNumber, language.Id, id)
		// query := fmt.Sprintf("SELECT %s FROM %s WHERE id=%s", table, field, id)
		fmt.Println(query)
		block := Block{}
		err = db.Get(&block, query)
...
func getAnswerFromTableIndistinctRu(db *sqlx.DB, id string, tableNumber int, sex string, short_opt ...bool) Block {
	if id != "" {
		// field := fmt.Sprintf("%s_%s", personal, lang)
		// table := fmt.Sprintf("answers%d", tableNumber)
		_, err := strconv.Atoi(id)
		...
		query := fmt.Sprintf("SELECT * FROM prediction WHERE type_id=%d AND id IN(SELECT prediction_id FROM predictionrel WHERE combination=%s)", tableNumber, id)
		// query := fmt.Sprintf("SELECT %s FROM %s WHERE id=%s", table, field, id)
		fmt.Println(query)
		block := Block{}
		err = db.Get(&block, query)
